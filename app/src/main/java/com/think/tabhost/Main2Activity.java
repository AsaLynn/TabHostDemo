...
public class Main2Activity extends TabActivity implements View.OnClickListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        //tabContentView.setBackgroundColor(Color.GREEN);

        String[] titles = {"杨坤", "欢欢", "那英", "哈林"};
        int[] ids = {R.id.alwayswet, R.id.isanimal, R.id.nezha, R.id.halin};
        for (int i = 0; i < titles.length; i++) {
            //创建并设置选项标签名称,以及对应布局视图
            TabHost.TabSpec page = tabHost.newTabSpec("tab" + i)
                    .setIndicator(titles[i], drawable)
                    .setContent(ids[i]);
            //添加选项
            tabHost.addTab(page);
        }
    ...
    public void onClick(View view) {
        if (view.getId() == R.id.iv0) {
            ...
            Toast.makeText(this, ""+currentTab, Toast.LENGTH_SHORT).show();
        }
    }

    private void initView() {
        iv0 = (ImageView) findViewById(R.id.iv0);
        iv0.setOnClickListener(Main2Activity.this);
    }
}
