        ...
        for (int i = 0; i < titles.length; i++) {
            ...
            TabHost.TabSpec page = tabHost.newTabSpec("tab" + i)
                .setIndicator(titles[i])
                    .setContent(ids[i]);
            if (i ==0){
                page.setContent(new Intent(this,IntentActivity.class));
            }else if (i ==1){
                page.setContent(new TabHost.TabContentFactory() {
                    @Override
                    public View createTabContent(String tag) {
                        TextView view = new TextView(Main1Activity.this);
                        view.setText("001");
                        return view;
                    }
                });
